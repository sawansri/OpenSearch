...
//                    return new IndexOrDocValuesQuery(query, dvQuery);
                    return new ApproximateScoreQuery(IntPoint.newExactQuery(field, v), new ApproximatePointRangeQuery(field, IntPoint.pack(new int[] { v }).bytes, IntPoint.pack(new int[] { v }).bytes, new int[] { v }.length, ApproximatePointRangeQuery.INT_FORMAT));
                }
                if (hasDocValues) {
